...
containers::list<int> BuildNonEmptyList(containers::list<int>::size_type size = SIZE) {
...
TEST_F(ListTestSuite, ShouldResizeWhenCountIsLesserThenListSizeAndDefinedValueAndJustCutAdditionalValues) {
    sut.resize(SIZE - 1, DEFAULT_VALUE + 100);
    ASSERT_EQ(sut, BuildNonEmptyList(SIZE - 1));
}

TEST_F(ListTestSuite, ShouldNotResizeListIfSizeAndNewSizeIsTheSameAndDefinedValue) {
...
TEST_F(ListTestSuite, ShouldResizeWhenCountIsLesserThenListSizeAndJustCutAdditionalValues) {
    sut.resize(SIZE - 1);
    ASSERT_EQ(sut, BuildNonEmptyList(SIZE - 1));
}
