...
containers::list<int> BuildListFilledWithZerosWithGivenSize(containers::list<int>::size_type size, int value = 0) {
...
containers::list<int> BuildListWithElementsStartingWith(containers::list<int>::value_type value, containers::list<int>::size_type count) {
    containers::list<int> result;
    for (containers::list<int>::size_type i = 0; i < count; ++i) {
...
public:
    void EvaluateList(
        list<int>::size_type size = EMPTY, const list<int>& expected = EXPECTED_EMPTY_LIST) {
...
TEST_F(EmptyListTestSuite, ShouldResizeAndFillWithDefinedvalue) {
    ...
    ASSERT_EQ(sut, BuildListFilledWithZerosWithGivenSize(SIZE, DEFAULT_VALUE - 10));
}
...
TEST_F(EmptyListTestSuite, ShouldAssignNewListUsingInitList) {
    sut.assign({DEFAULT_VALUE, DEFAULT_VALUE + 1, DEFAULT_VALUE + 2, DEFAULT_VALUE + 3, DEFAULT_VALUE + 4});
